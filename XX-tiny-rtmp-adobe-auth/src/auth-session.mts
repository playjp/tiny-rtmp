...
export default class AdobeAuthSession {
  ...
  public query(): string {
    ...
    return `user=${encodeURIComponent(this.user)}&salt=${this.session.salt.toString('base64')}&challenge=${this.session.challenge.toString('base64')}&opaque=${this.session.opaque.toString('base64')}`;
  }
  ...
  public verify(response: string, challenge: string): boolean {
    ...
    const secondstep = crypto.createHash('md5').update(firststep).update(this.session.opaque.toString('base64')).update(challenge).digest('base64');

...
import read_message, { MessageType, type Message } from '../../01-tiny-rtmp-server/src/message-reader.mts';
...
const TRANSITION = {
  [STATE.WAITING_CONNECT]: (message: Message, builder: MessageBuilder, connection: Duplex, handers: AuthHandler, auth: AdobeAuthSession, context: RTMPContext) => {
    ...
    const [data, result, next_context] = handers.connect(transaction_id, appName, auth, context);
    const next = {
  ...
  [STATE.WAITING_CREATESTREAM]: (message: Message, builder: MessageBuilder, connection: Duplex, handers: AuthHandler,  auth: AdobeAuthSession, context: RTMPContext) => {
  ...
  [STATE.WAITING_PUBLISH]: (message: Message, builder: MessageBuilder, connection: Duplex, handers: AuthHandler, auth: AdobeAuthSession, context: RTMPContext) => {
    ...
    const [data, result, next_context] = handers.publish(transaction_id, streamKey, auth, context);
    const next = {
  ...
  [STATE.PUBLISHED]: (message: Message, builder: MessageBuilder, connection: Duplex, handers: AuthHandler, auth: AdobeAuthSession, context: RTMPContext) => {
  ...
  [STATE.DISCONNECTED]: (message: Message, builder: MessageBuilder, connection: Duplex, handers: AuthHandler,  auth: AdobeAuthSession, context: RTMPContext) => {
    return [STATE.DISCONNECTED, context];
  },

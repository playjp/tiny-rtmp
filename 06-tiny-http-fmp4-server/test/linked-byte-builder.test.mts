...
import LinkedByteBuilder from '../src/linked-byte-builder.mts';

describe('Unit Test', () => {
  test('Build empty buffer in initial state', () => {
    const target = new LinkedByteBuilder();
  ...
  test('Write mixed data and Iterate Then', () => {
    const target = new LinkedByteBuilder();
    ...
    expect(result[0].equals(Buffer.from([0x01]))).toStrictEqual(true);
  });

  test('Write mixed data and Combine Then', () => {
    const target = new LinkedByteBuilder();
    target.writeU8(0x01);

    const target2 = new LinkedByteBuilder();
    target2.writeU8(0x02);

    target.append(target2);

    const result = Array.from(target);
    expect(result[0].equals(Buffer.from([0x01]))).toStrictEqual(true);
    expect(result[1].equals(Buffer.from([0x02]))).toStrictEqual(true);
  });

  test('Write mixed data and Combine null builder', () => {
    const target = new LinkedByteBuilder();
    target.writeU8(0x01);
    target.writeU8(0x02);
